...
import threading
from .models import (
    ProjectContext, AgentOutput, WorkflowState, ProjectPlan, 
    ModuleStructure, TestPlan, CodeArtifact, TestResult
)
...
class SharedDataStore:
    ...
    def store_agent_output(self, agent_name: str, output_type: str, data: Dict[str, Any], 
        ...
        with self._lock:
            ...
            self._log_operation("store_output", {
                "agent_name": agent_name,
                "output_type": output_type,
    ...
    def get_agent_output(self, agent_name: str, output_type: Optional[str] = None) -> List[AgentOutput]:
        """
    ...
    def get_latest_agent_output(self, agent_name: str, output_type: Optional[str] = None) -> Optional[AgentOutput]:
        """
    ...
    def update_workflow_state(self, workflow_state: WorkflowState) -> None:
        ...
        with self._lock:
            ...
            self._log_operation("update_workflow_state", {
    ...
    def _update_project_context(self, output_type: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to update project context for output type {output_type}: {e}")
            ...
            if outputs_file.exists():
                with open(outputs_file, 'r') as f:
                    ...
                    for agent_name, outputs in outputs_data.items():
                        self._agent_outputs[agent_name] = [AgentOutput(**output) for output in outputs]
