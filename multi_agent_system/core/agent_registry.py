...
class AgentRegistry:
    ...
    def get_execution_order(self) -> List[str]:
        ...
        def visit(agent_name: str):
            if agent_name in temp_visited:
                raise ValueError(f"Circular dependency detected involving agent {agent_name}")
            ...
            if agent_name not in visited:
                ...
                for dep in self._dependencies.get(agent_name, []):
                    if dep not in self._agents:
                        raise ValueError(f"Dependency {dep} for agent {agent_name} not found")
                    visit(dep)
    ...
    def validate_dependencies(self) -> bool:
        ...
        for agent_name, deps in self._dependencies.items():
            for dep in deps:
                if dep not in self._agents:
                    raise ValueError(f"Agent {agent_name} depends on {dep} which is not registered")
