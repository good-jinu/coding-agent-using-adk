    ...
    try:
        ...
        class CriticalFailingAgent(BaseMultiAgent):
            ...
            def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
                ...
                raise AgentExecutionError("ProjectPlanningAgent", "Critical failure for testing")
        ...
        if not result.success:
            ...
            if "ProjectPlanningAgent" in result.failed_agents:
                logger.info("Critical agent correctly marked as failed")
                return True
            else:
                logger.error("Critical agent not marked as failed")
                return False
        else:
            logger.error("Workflow should have failed due to critical agent failure but didn't")
            return False
...
def test_retry_mechanism():
    ...
    class SometimesFailingAgent(BaseMultiAgent):
        ...
        def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if self.attempt_count < 3:  # Fail first 2 attempts
                raise AgentExecutionError("SometimesFailingAgent", f"Failure on attempt {self.attempt_count}")
    ...
    try:
        ...
        coordinator = MultiAgentCoordinator(registry=retry_registry, data_store=retry_data_store, max_retries=3, retry_delay=0.1)
        ...
        if result.success:
            ...
            logger.error("Workflow should have succeeded after retries")
            return False
...
if __name__ == "__main__":
    ...
    # Test error handling
    test1_success = test_error_handling()
